...
func main() {
	...
	go func() {
		...
		defer release()
		if err := application.shutdown(ctx, server); err != nil {
...
func buildApplication(cfg config.Config, logger *slog.Logger) (*runtime, func(), error) {
	...
	go runScheduler(schedulerContext, schedulerDone, scheduler.Run, logger)
	return &runtime{handler: handler, schedulerCancel: schedulerCancel, schedulerDone: schedulerDone}, func() { schedulerCancel() }, nil
}

func runScheduler(ctx context.Context, done chan<- struct{}, run func(context.Context) error, logger *slog.Logger) {
	if err := run(ctx); err != nil && !errors.Is(err, context.Canceled) && logger != nil {
