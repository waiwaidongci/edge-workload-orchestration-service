...
func (t *Template) AddVersion(version Version, now time.Time) (Version, error) {
	...
	version.Environment = cloneMap(version.Environment)
...
func copyVersion(source Version) Version {
	source.Command = append([]string(nil), source.Command...)
	source.RequiredCapabilities = append([]string(nil), source.RequiredCapabilities...)
	source.Environment = cloneMap(source.Environment)
	return source
}
