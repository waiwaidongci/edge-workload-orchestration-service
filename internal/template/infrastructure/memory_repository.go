...
func clone(item *templatedomain.Template) *templatedomain.Template {
	...
	for i, version := range item.Versions {
		...
		copy.Versions[i].Command = append([]string(nil), version.Command...)
		copy.Versions[i].RequiredCapabilities = append([]string(nil), version.RequiredCapabilities...)
		copy.Versions[i].Environment = map[string]string{}
		for k, v := range version.Environment {
			copy.Versions[i].Environment[k] = v
		}
	}
	return &copy
}
